...
exports.generateTrainingReport = async (athleteData, workouts, performance) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const fileName = `training-report-${athleteData._id}-${Date.now()}.pdf`;
      ...
      doc.fontSize(20).text('Training Performance Report', { align: 'center' });
      doc.moveDown();
      ...
      doc.fontSize(16).text('Workout Summary', { underline: true });
      doc.moveDown();
      ...
      if (workouts.length > 0) {
        doc.fontSize(14).text('Recent Workouts:', { underline: true });
        doc.moveDown();
      ...
      doc.moveDown(2);
      doc.fontSize(10).text('This report was automatically generated by Smart Coaching & Training Planner', { align: 'center', color: 'gray' });
...
exports.generatePlanReport = async (plan, athletesData) => {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ margin: 50 });
      const fileName = `plan-report-${plan._id}-${Date.now()}.pdf`;
      const filePath = path.join(__dirname, '../reports', fileName);
      ...
      doc.fontSize(20).text('Training Plan Report', { align: 'center' });
      doc.moveDown();
      ...
      if (athletesData && athletesData.length > 0) {
        doc.fontSize(14).text(`Assigned Athletes (${athletesData.length}):`, { underline: true });
        doc.moveDown();
      ...
      if (plan.workouts && plan.workouts.length > 0) {
        doc.addPage();
        doc.fontSize(16).text('Workout Schedule', { underline: true });
        doc.moveDown();
        ...
        plan.workouts.forEach((workout, index) => {
          ...
          if (workout.restDay) {
            doc.fontSize(11).text('   REST DAY', { color: 'blue' });
